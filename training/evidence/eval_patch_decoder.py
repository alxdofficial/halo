...
from model.evidence.confidence import EvidenceConfidenceHead
from model.evidence.decoder import DecoderConfig, EvidenceDecoder
from model.evidence.edl import acc_at_coverage, aurc
...
def score_cell(
    ...
    *,
    ...
    for start in range(0, len(es.windows), batch):
        ...
        logits, conf_logit, _aux = decode_patch_queries(
            dec, retriever, confidence, bank, index_rows, memory_index,
        ...
        confidence_score[start:stop] = torch.sigmoid(conf_logit).cpu().numpy()
...
def main() -> None:
    ...
    ap.add_argument("--decoder", type=Path, default=_DIR / "patch_evidence_decoder.pt")
    ...
    blob = torch.load(args.decoder, map_location="cpu", weights_only=True)
    assert_artifact_matches_bank(
        blob, bank, context="eval_patch_decoder", artifact_name="patch evidence decoder"
    )
    ...
    confidence.load_state_dict(blob["confidence"])
    ...
    for dataset in args.datasets:
        for spec in policy.stream_specs(dataset, "primary"):
            ...
            if result is None:
                continue
            f1, cell_calibration = result
            key = f"{dataset}/{spec.stream_id}"
            per_cell[key] = round(f1, 1)
            calibration[key] = cell_calibration
            print(f"  {key:38s} F1={f1:.1f} AURC={cell_calibration['aurc']:.3f}", flush=True)
    ...
    out.write_text(json.dumps({
        "per_cell": per_cell, "mean": mean, "calibration": calibration,
