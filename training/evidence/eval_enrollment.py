...
def _support_and_query_rows(
    ...
    *,
    ...
    for position, label in enumerate(candidate_names):
        ...
        if len(source) < support_count:
            raise ValueError(f"{label}: insufficient {mode} enrollment support")
        chosen = np.asarray(rng.choice(source, size=support_count, replace=False), dtype=np.int64)
        support.extend(chosen.tolist())
        support_position.extend([position] * support_count)
        remaining = same[~np.isin(same, chosen)] if mode == "same_subject" else same
...
def score_enrollment_cell(
    ...
    *,
    ...
    for subject_index, subject in enumerate(np.unique(subjects)):
        same_subject_labels = {
            label for label in eval_labels
            if np.sum((labels == label) & (subjects == subject)) >= support_count + 1
        }
        if mode == "cross_subject":
            same_subject_labels = {
                label for label in same_subject_labels
                if np.sum((labels == label) & (subjects != subject)) >= support_count
            }
        ...
        support, support_position, query_rows = _support_and_query_rows(
            labels, subjects, subject, candidate_names,
        ...
        for start in range(0, len(query_rows), batch_size):
            ...
            logits, _ = decode_adaptation_episode(
...
def main() -> None:
    ...
    if any(value < 1 for value in args.support):
        parser.error("support counts must be positive")
    ...
    for dataset in args.datasets:
        for stream_spec in deployment_policy.stream_specs(dataset, "primary"):
            try:
                stream = load_eval_stream(dataset, stream_spec.stream_id, alignment="non_harmonised")
            ...
            for mode in args.modes:
                for support_count in args.support:
                    result = score_enrollment_cell(
                        encoded, labels, subjects, list(stream.eval_labels), bank,
                    ...
                    results[key] = result
                    print(f"{key}: F1={result['f1_macro']:.1f} n={result['queries']}", flush=True)
