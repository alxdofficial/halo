"""Run the complete Task-3 evaluation for one frozen encoder cache.

Task 1 is evaluated by ``applications.motion_monitoring.task1.evaluate_v2`` under
its own cohort (spec section F); this runner no longer carries a Task-1 path.
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--cohort", type=Path, default=root / "manifests/COHORT_TASK3_V2.json"
    )
    parser.add_argument(
        "--task3-manifest", type=Path, default=root / "manifests/TASK3_TEST_V2.json"
    )
    ...
    datasets = sorted({str(row["dataset"]) for row in task3_manifest.units})
    ...
    # Thresholds were fixed before the encoder ever saw the test manifest, on
    # held-out TRAINING identities under a declared false-edge budget.
    operating_point = task3_training["operating_point"]
    thresholds = {}
    for readout in ("direct_cosine_recurrence", "learned_metric_recurrence"):
        point = operating_point[readout]
        if "threshold" not in point:
            raise ValueError(
                f"{readout} has no fixed operating point: {point.get('unsupported')}"
            )
        thresholds[readout] = float(point["threshold"])
    ...
    task3_direct = evaluate_task3_test(
        ...
        recording_caches,
        representations,
        score_threshold=thresholds["direct_cosine_recurrence"],
    ...
    task3_learned = evaluate_task3_test(
        ...
        recording_caches,
        representations,
        score_threshold=thresholds["learned_metric_recurrence"],
        model=task3_model,
        durations_sec=task3_checkpoint["durations"],
        candidate_stride_sec=float(task3_checkpoint["candidate_stride"]),
        block_size=args.task3_block_size,
    ...
    task3_protocol_base = {
        ...
        "train_manifest_fingerprint": task3_training.get("train_manifest_fingerprint"),
        "threshold_selection": "a_priori_false_edge_budget_on_holdout_identities",
        "operating_point_protocol": task3_training["operating_point_protocol"],
    ...
    for readout, results in (
        ("direct_cosine_recurrence", task3_direct),
        ("learned_metric_recurrence", task3_learned),
    ):
        _write_records(
            output=args.output,
            task="task3",
